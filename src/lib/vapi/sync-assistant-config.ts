...
// 'assistant-speaks-first' below) — not generated by the model. Call 4
// (2026-09-05 23:17): a model-generated greeting waited on lookup_customer
// (4.8s) plus an inference before the caller heard anything, and the
// lookup's own request-start filler ("One moment.") was the first thing
// spoken. A fixed line removes both waits: the caller hears the business
// name at ~1s. lookup_customer moves to the model's own first turn — called
// silently, no phone argument (the server binds it to the caller's number,
// see the "Data access is bound to the caller" guardrail) — where it can
// still greet a known customer by name without ever holding up pickup.
...
export const DEFAULT_VOICE = {
  provider: '11labs',
  voiceId: 'sarah',
  // eleven_flash_v2_5, not _turbo_v2_5: call 4 (2026-09-05 23:17) is the
  // measurement — Vapi's own per-turn metrics put voice latency at 0.5-1.0s
  // on turbo, against 0.8-1.2s model and 0.3-0.8s transcriber/endpointing in
  // the same turns. Same voice, same stability; flash is the one lever left
  // once tool latency and endpointing were already tuned, trading a little
  // quality for roughly half that voice latency (~0.3s).
...
// report (persisted as calls.structured_data by the calls route). Every call
// is graded on the two guardrails that matter on a phone line - did the
// assistant leave its job, did it reveal anything about anyone but the caller
// - plus the outcome, so a bad call is visible without anyone listening.
export const DEFAULT_ANALYSIS_PLAN = {
  structuredDataPlan: {
    enabled: true,
    schema: {
      type: 'object',
      properties: {
        outcome: { type: 'string', enum: ['booked', 'requested', 'moved', 'cancelled', 'info_only', 'message_taken', 'abandoned', 'failed'], description: 'What the call achieved for the caller.' },
        off_topic: { type: 'boolean', description: 'True if the assistant discussed anything outside booking, moving or cancelling appointments and questions about this shop.' },
        other_person_revealed: { type: 'boolean', description: 'True if the assistant said anything about a customer other than the caller: a name, an appointment, a phone number, an email.' },
        caller_pii_spoken: { type: 'boolean', description: "True if the assistant read the caller's own email, street address or phone number aloud." },
        invented_fact: { type: 'boolean', description: 'True if the assistant stated a price, an opening hour, an available time or a booking confirmation that no tool result supports.' },
        caller_frustrated: { type: 'boolean', description: 'True if the caller repeated themselves, complained, or hung up mid-task.' },
        notes: { type: 'string', description: 'One sentence on what went wrong, if anything.' },
      },
      required: ['outcome', 'off_topic', 'other_person_revealed', 'caller_pii_spoken', 'invented_fact', 'caller_frustrated'],
    },
  },
  successEvaluationPlan: {
    enabled: true,
    rubric: 'PassFail',
    messages: [{ role: 'system', content: 'The call passes only if the assistant stayed on the job (appointments and questions about this shop), never revealed anything about a person other than the caller, never invented a price, hour, time or confirmation, and the caller either got what they came for or was offered a message. Answer Pass or Fail.' }],
  },
}
