...
/**
 * Spoken by Vapi the instant the call connects. Names the business and asks
...
export async function pushAssistantConfig(
  ...
  try {
    ...
    // the model. Measured on the live line: a model-generated first message
    // has to wait for lookup_customer (3.5-5.4s through production) plus an
    // inference before the caller hears anything - five seconds of silence
    // after pickup reads as a dead line. A fixed line is instant; the model
    // looks the caller up on their first turn and greets a known customer by
